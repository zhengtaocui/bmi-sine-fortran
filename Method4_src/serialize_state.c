    ...
    for (i=0; i<n_state_vars; i++){
        ...
        if (length == 1){
            if (strcmp(type, "int") == 0 || 
               ...
               //fortran logical by default is 4 byte
               *(int*)ptr_list[i] ? msgpack_pack_true(&pk ) : 
		                               msgpack_pack_false(&pk );
            } else if ( strcmp(type, "bool") == 0 ){
               //C bool is 1 byte
               *(bool*)ptr_list[i] ? msgpack_pack_true(&pk ) : 
		                               msgpack_pack_false(&pk );
    ...
    while (ret == MSGPACK_UNPACK_SUCCESS) {
        ...
        if (length == 1){
            ...
            if (strcmp(type, "int") == 0 || strcmp(type, "integer4") == 0 ){
                ...
                model2->set_value(model2, name, &obj.via.i64 );
            } else if ( strcmp(type, "bool") == 0 || strcmp(type, "logical" ) == 0 ) {
                b_val = (bool)obj.via.boolean;
                model2->set_value(model2, name, &b_val );
            } else{
...
int compare_states(Bmi* model1, Bmi* model2){
    ...
    for (int i=0; i<n_state_vars; i++){
        ...
        if (strcmp(type, "int") == 0 || strcmp(type, "integer4") == 0){
            ...
            for (j=0; j<length; j++){
                //Fortran logical is 4 byte by default
                i_val1 = *( ((int *)ptr_list1[i]) + j);
                i_val2 = *( ((int *)ptr_list2[i]) + j);
                if (i_val1 != i_val2){
                    printf("Mismatch: i = %d, j = %d\n", i, j);
    ...
    if ( err_count > 0 ) return BMI_FAILURE;
    return 0;
}
